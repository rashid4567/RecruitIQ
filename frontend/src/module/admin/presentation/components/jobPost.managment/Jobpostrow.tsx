import {
  Briefcase, MapPin, Eye, DollarSign, Clock,
  MoreVertical, Ban, CheckCircle, Globe, Users,
  Zap, Building2,
} from "lucide-react";
...
export function StatusBadge({ status, isBlocked }: { status: JobStatus; isBlocked: boolean }) {
  if (isBlocked) {
    return (
  ...
  const cfg: Record<JobStatus, { cls: string; dot?: string; label: string }> = {
    active:  { cls: "bg-emerald-50 text-emerald-700 border-emerald-200 ring-emerald-100", dot: "bg-emerald-500", label: "Active" },
    draft:   { cls: "bg-sky-50 text-sky-700 border-sky-200 ring-sky-100",                label: "Draft" },
    expired: { cls: "bg-amber-50 text-amber-700 border-amber-200 ring-amber-100",        label: "Expired" },
  };
  const c = cfg[status] ?? cfg.draft;
  return (
    <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ring-1", c.cls)}>
      {c.dot && <span className={cn("w-1.5 h-1.5 rounded-full animate-pulse", c.dot)} />}
      {c.label}
    </span>
  );
}
...
export function TypeBadge({ jobType, isRemote }: { jobType: JobType; isRemote: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className={cn("inline-flex items-center px-2.5 py-0.5 rounded-md text-[11px] font-semibold border capitalize w-fit", typeStyles[jobType])}>
        {jobType.replace("-", " ")}
      </span>
      {isRemote && (
...
export function SkillBadges({ skills, max = 3 }: { skills: string[]; max?: number }) {
...
export function ApplicationsBar({ count }: { count: number }) {
  ...
  const color =
    pct < 30 ? "from-emerald-400 to-emerald-500" :
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
        <div
          className={cn("h-full rounded-full bg-linear-to-r transition-all duration-500", color)}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
...
export function JobPostRow({ job, onView, onToggleBlock, isPending }: JobPostRowProps) {
  return (
    <tr
      className={cn(
    ...
    >
      ...
      {/* Posted */}
      <td className="px-5 py-4">
        <p className="text-sm font-semibold text-slate-900">{formatDate(job.postedOn)}</p>
      ...
      <td className="px-6 py-4 text-right">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48 rounded-2xl shadow-xl border-slate-200 p-1.5">
            <DropdownMenuItem
              onClick={() => onView(job)}
            ...
            >
              {job.isBlocked ? (
                <><CheckCircle className="w-4 h-4" /><span className="font-medium text-sm">Unblock Job</span></>
              ) : (
                <><Ban className="w-4 h-4" /><span className="font-medium text-sm">Block Job</span></>
              )}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
