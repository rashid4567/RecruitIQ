...
import { getJobById, updateJob, publishJob } from "@/module/jobs/api/job.api";

import type { UpdateJobPostDTO } from "@/module/jobs/types/updateJobPost.dto";
...
export function useUpdateJobPost(jobId: string) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const job = await getJobById("recruiter", jobId);
        setFormData({
  ...
  const buildDto = useCallback((): UpdateJobPostDTO => {
    return {
  ...
  const confirmSaveDraft = useCallback(async () => {
    ...
    try {
      await updateJob("recruiter", jobId, buildDto());
  ...
  const confirmPublish = useCallback(async () => {
    ...
    try {
      ...
      await publishJob("recruiter", jobId);
