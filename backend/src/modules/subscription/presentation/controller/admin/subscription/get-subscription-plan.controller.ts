...
export class GetSubscriptionPlanController {
  constructor(
    private readonly _getAllPlansUC: IUseCase<
      GetAllPlansRequestDTO,
      GetAllPlansResponseDTO
    >,
  ) {}
  ...
  getAllPlans = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { data, total } = await this._getAllPlansUC.execute(filter);
      ...
      return ApiResponse.success(
        res,
        HTTP_STATUS.OK,
        SUCCESS_MESSAGES.SUBSCRIPTION_PLANS_FETCHED_SUCCESSFULLY,
        {
          data,
          total,
          page: pageValue,
