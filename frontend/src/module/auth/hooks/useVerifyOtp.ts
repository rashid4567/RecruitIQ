...
import { sendOtp, verifyOtp as verifyOtpApi } from "../api/auth.api";
...
function extractAxiosError(err: unknown): {
  ...
  const axiosResponse = (err as AxiosLikeError).response;

  if (axiosResponse) {
    const status: number = axiosResponse.status ?? 0;

    const serverMessage: string =
      axiosResponse.data?.message ?? axiosResponse.data?.error ?? "";

    return {
      status,
      message: serverMessage.toLowerCase(),
    };
  }

  const axiosMessage = (err as AxiosLikeError).message?.toLowerCase?.() ?? "";
...
function classifyError(err: unknown): string {
  ...
  if (
    message.includes("otp_already_used") ||
...
export function useVerifyOtp() {
  ...
  const resendOtp = async () => {
    ...
    try {
      await sendOtp({ email: state.email, role: state.role });
      setOtp(Array(6).fill(""));
