...
export default function PlanCard({
  ...
  return (
    <div
      className={`group relative flex flex-col overflow-hidden rounded-2xl border bg-white transition-all duration-300 hover:shadow-2xl ${plan.isRecommended ? "border-2 border-amber-200 shadow-xl shadow-amber-500/20" : "border-zinc-200 hover:border-zinc-300"}`}
    >
      {plan.isRecommended && (
      ...
      <div className="p-6 pb-0">
        <div className="flex items-start justify-between">
          ...
          >
            <span
              className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-all ${plan.isActive ? "translate-x-5.5" : "translate-x-0.5"}`}
            />
          </button>
        </div>
        ...
        <div className="space-y-3">
          {plan.features.slice(0, 5).map((feature, idx) => (
            <div key={idx} className="flex items-center gap-3">
              ...
              <span
                className={`text-sm ${feature.included ? "text-zinc-700" : "text-zinc-400 line-through"}`}
              >
                {feature.name}
              </span>
            </div>
