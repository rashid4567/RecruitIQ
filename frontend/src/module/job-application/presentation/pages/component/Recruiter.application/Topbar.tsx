import { Bell, Plus, RefreshCw, ChevronLeft, Briefcase } from "lucide-react";
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-6 h-14 flex items-center justify-between shrink-0">
      <div className="flex items-center gap-1 text-sm">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-1.5 text-slate-400 hover:text-slate-700 px-2 py-1.5 rounded-lg hover:bg-slate-100 transition font-medium"
        >
          <ChevronLeft size={14} />
          Back
        </button>
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-slate-500 hover:text-slate-800 px-2 py-1.5 rounded-lg hover:bg-slate-100 transition font-medium"
        >
          <Briefcase size={13} />
          Manage Jobs
        </button>
