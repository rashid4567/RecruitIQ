...
import type { RescheduleInterviewRequest, ScheduleInterviewRequest } from '../types/recruiterInterview.types';
...
interface MeetingModeFields {
  mode: InterviewMode;
  meetingLinkOption: 'later' | 'paste';
  meetingLink?: string;
  location?: string;
}

function refineMeetingModeFields(data: MeetingModeFields, ctx: z.RefinementCtx) {
...
export const rescheduleStepFields: Record<number, Array<keyof RescheduleFormValues>> = {
...
export function splitIsoToLocalParts(iso: string): {
  ...
  return {
    ...
    hour: pad(d.getHours()),
    minute: pad([0, 15, 30, 45].reduce((closest, m) =>
      Math.abs(m - d.getMinutes()) < Math.abs(closest - d.getMinutes()) ? m : closest,
    )),
  };
}