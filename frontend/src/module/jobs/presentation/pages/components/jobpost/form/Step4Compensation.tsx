...
import { DollarSign, Calendar, Link as LinkIcon, IndianRupee, Euro, PoundSterling } from "lucide-react";
...
export default function Step4Compensation({ formData, setFormData, errors }: Props) {
  ...
  const minDisplay = formatSalaryDisplay(formData.salary.min, selectedCurrency.symbol);
  const maxDisplay = formatSalaryDisplay(formData.salary.max, selectedCurrency.symbol);
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Compensation & Details</h2>
        </div>
        <p className="text-gray-500 text-sm ml-10">Set salary expectations and application details</p>
      </div>
      ...
      <div className="space-y-7">
              ...
              {currencies.map((c) => {
                ...
                return (
                  <button
                    key={c.code}
                    type="button"
                    onClick={() => setFormData((p) => ({ ...p, salary: { ...p.salary, currency: c.code } }))}
                  ...
                  >
                    <Icon className={`w-4 h-4 mb-1 ${isActive ? "text-emerald-600" : "text-gray-400"}`} />
                    <span className="text-xs font-bold">{c.code}</span>
                    <span className="text-[10px] text-gray-400 font-normal">{c.symbol}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, salary: { ...p.salary, min: parseInt(e.target.value) || 0 } }))
                  }
                  placeholder="0"
                  className={`h-12 pl-8 rounded-xl border-2 bg-white text-sm font-medium transition-all focus:ring-4 focus:ring-emerald-100 ${
                    errors["salary.min"] ? "border-red-400" : "border-emerald-200 focus:border-emerald-400"
                  }`}
                />
                {minDisplay && (
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, salary: { ...p.salary, max: parseInt(e.target.value) || 0 } }))
                  }
                  placeholder="0"
                  className={`h-12 pl-8 rounded-xl border-2 bg-white text-sm font-medium transition-all focus:ring-4 focus:ring-emerald-100 ${
                    errors["salary.max"] ? "border-red-400" : "border-emerald-200 focus:border-emerald-400"
                  }`}
                />
                {maxDisplay && (
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-emerald-600 pointer-events-none">
        ...
        <div>
          ...
          <div className="relative">
            <Input
              ...
              className={`h-12 rounded-xl border-2 text-sm font-medium transition-all focus:ring-4 focus:ring-rose-50 cursor-pointer ${
                errors.expiresAt
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1.5 ml-1">
            Candidates will be redirected here to apply. Leave blank to use the built-in application flow.
          </p>
        </div>
      </div>
    </div>
