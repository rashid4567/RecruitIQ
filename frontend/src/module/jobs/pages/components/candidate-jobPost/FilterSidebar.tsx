...
import type { JobType } from "@/module/jobs/types/jobPost.dto";
...
const JOB_TYPES: { value: JobType; label: string; color: string }[] = [
  { value: "full-time", label: "Full-time", color: "bg-emerald-100 text-emerald-700 border-emerald-200" },
  { value: "part-time", label: "Part-time", color: "bg-blue-100 text-blue-700 border-blue-200" },
  { value: "contract", label: "Contract", color: "bg-amber-100 text-amber-700 border-amber-200" },
  { value: "internship", label: "Internship", color: "bg-purple-100 text-purple-700 border-purple-200" },
];

function isExperienceSelected(filters: JobPostFilters, level: ExperienceLevel): boolean {
  if (filters.experienceMin === undefined || filters.experienceMax === undefined) return false;
  return filters.experienceMin === level.min && filters.experienceMax === level.max;
}
  ...
  >
    ...
    <span
      ...
      className={`w-4 h-4 rounded-[5px] border-2 flex items-center justify-center shrink-0 transition-all duration-150
        ${checked ? "bg-indigo-600 border-indigo-600 shadow-sm shadow-indigo-300" : "border-slate-300 bg-white"}`}
    >
      {checked && (
    ...
    <span className="flex-1 flex items-center justify-between gap-2">
      <span className={`text-sm font-medium transition-colors ${checked ? "text-indigo-700" : "text-slate-600"}`}>
        {label}
      </span>
      {sublabel && (
        <span className={`text-xs transition-colors ${checked ? "text-indigo-400" : "text-slate-400"}`}>
          {sublabel}
        </span>
      )}
    </span>
...
const JobTypePill: React.FC<{
  ...
  <label
    ...
    className={`inline-flex items-center px-3 py-1.5 rounded-full border text-xs font-semibold cursor-pointer transition-all duration-150 select-none
      ${checked ? `${color} shadow-sm scale-105` : "bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700"}`}
  >
    <input type="checkbox" id={`type-${value}`} checked={checked} onChange={onChange} className="sr-only" />
    {label}
  </label>
);
...
export const FilterSidebar: React.FC<FilterSidebarProps> = ({
  ...
  const handleDepartmentChange = (dept: string) => {
    onFilterChange({ department: filters.department === dept ? undefined : dept });
  };
  ...
  return (
    // NOTE: no max-h / overflow-y-auto here — a sticky element must NOT own its
    // own scroll container, or it creates a second, nested scrollbar (the bug
    // you saw). The page's single scroll container handles everything.
    <aside className={`w-60 shrink-0 ${className}`}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden px-4 divide-y divide-slate-100">
        ...
        <FilterSection title="Experience" badge={filters.experienceMin !== undefined ? 1 : 0}>
          <div className="space-y-0.5">
        ...
        <FilterSection title="Job Type" badge={(filters.jobType ? 1 : 0) + (filters.isRemote ? 1 : 0)}>
          ...
          <label
            ...
            className={`flex items-center justify-between px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-150 select-none
              ${filters.isRemote ? "bg-indigo-50 ring-1 ring-indigo-200" : "hover:bg-slate-50"}`}
          >
            <input
              type="checkbox"
              id="remote-toggle"
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className={`text-sm font-medium ${filters.isRemote ? "text-indigo-700" : "text-slate-600"}`}>
                Remote Only
              </span>
            </div>
            <span
      ...
      {activeCount > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3 px-1">
          ...
          {filters.experienceMin !== undefined && (
            <button
              type="button"
              onClick={() => onFilterChange({ experienceMin: undefined, experienceMax: undefined })}
              className="flex items-center gap-1 px-2.5 py-1 bg-indigo-50 border border-indigo-200 rounded-full text-xs text-indigo-700 font-medium hover:bg-indigo-100 transition-colors"
            >
              {EXPERIENCE_LEVELS.find((l) => l.min === filters.experienceMin)?.label}
              <X className="w-3 h-3" />
            </button>
          )}
          {filters.isRemote && (
