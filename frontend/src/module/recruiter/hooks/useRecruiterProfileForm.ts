...
import { updateRecruiterProfile } from "../api/recruiter.api";
import {
  profileSchema,
  type ProfileFormData,
} from "../validators/recruiter-form.validator";
import { RecruiterProfileFormMapper } from "../mapper/recruiterForm.mapper";
...
export function useRecruiterProfileForm({
  ...
  const submit = handleSubmit(async (data) => {
    try {
      ...
      const savedProfile = await updateRecruiterProfile(payload);
