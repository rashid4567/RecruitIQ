...
const steps = [
  {
    number: 1,
    title: "Basic Info",
    description: "Title, location, type",
    icon: Info,
  },
  {
    number: 2,
    title: "Description",
    description: "Role overview & duties",
    icon: FileText,
  },
  {
    number: 3,
    ...
    <div className="w-72 shrink-0">
      <div className="sticky top-8">
        <div className="bg-white rounded-3xl border border-gray-100 shadow-lg shadow-gray-100/80 overflow-hidden">
          <div className="px-6 pt-6 pb-5 bg-linear-to-br from-indigo-600 to-violet-600">
            ...
            <p className="text-indigo-200 text-xs mt-2">
              {totalCompleted === 4
                ? "Ready to publish! 🚀"
                : `${4 - totalCompleted} step${4 - totalCompleted !== 1 ? "s" : ""} remaining`}
            </p>
          </div>
          ...
          <div className="p-4 space-y-1">
            {steps.map((step) => {
              ...
              const isClickable =
                isCompleted ||
                step.number <= Math.max(...Array.from(completedSteps), 0) + 1;
          ...
          <div className="mx-4 mb-4 px-3 py-2.5 bg-amber-50 rounded-2xl border border-amber-100">
            <p className="text-xs text-amber-700 font-medium">
              💡 Use{" "}
              <kbd className="px-1 py-0.5 bg-amber-100 rounded text-amber-600 font-mono text-xs">
                →
              </kbd>{" "}
              <kbd className="px-1 py-0.5 bg-amber-100 rounded text-amber-600 font-mono text-xs">
                ←
              </kbd>{" "}
              keys to navigate steps
            </p>
          </div>
        </div>
      </div>
