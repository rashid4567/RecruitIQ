...
import type { JobStatus, JobType } from "@/module/jobs/types/jobPost.dto";
...
function formatSalary(s: Job["salary"]) {
  ...
  if (s.max == null) {
    return `${fmt.format(s.min ?? 0)}+`;
  }

  if (s.min == null) {
    return `Up to ${fmt.format(s.max)}`;
  }

  return `${fmt.format(s.min)} – ${fmt.format(s.max)}`;
}
function locationLabel(loc: Job["location"], isRemote: boolean): string {
