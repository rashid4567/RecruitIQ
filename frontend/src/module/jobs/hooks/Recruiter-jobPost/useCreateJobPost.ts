...
import { createJob, publishJob } from "@/module/jobs/api/job.api";
...
export function useCreateJobPost() {
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (!savedDraftId.current) {
        const created = await createJob("recruiter", dto);
  ...
  const confirmPublish = useCallback(async () => {
    ...
    try {
      const dto = buildDtoFromForm(formData);
      if (savedDraftId.current) {
        await publishJob("recruiter", savedDraftId.current);
      } else {
        const created = await createJob("recruiter", dto);

        savedDraftId.current = created.id;

        await publishJob("recruiter", created.id);
      }
