...
// Import Razorpay Hook
import { useRazorpay } from "../hooks/subscriptions/useRazorpay";    // ← Change path if your hook is located elsewhere
...
function getPlanCTA(plan: SubscriptionPlan): string {
  ...
  if (plan.planType === "enterprise") return "Contact Sales";
...
export default function PricingPlans() {
  ...
  // Razorpay Hook
  const { status, error, isLoading: paymentLoading, initiatePayment } = useRazorpay({
  ...
  const handleSubscribe = async () => {
    ...
    if (selectedPlan.planType === "enterprise") {
      alert("Enterprise Plan: Please contact sales team.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-blue-50/30 to-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {plans.map((plan) => {
              ...
              return (
                ...
                >
                  {/* ... Your entire plan card code remains exactly the same ... */}
                  {plan.isPopular && (
                  ...
                  <div className={`rounded-2xl p-6 lg:p-8 h-full ${isHighlighted ? "bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-700 text-white" : "bg-white border border-slate-200"}`}>
                    {/* Card content unchanged - same as your original */}
      ...
      {/* Selected Plan Detail with Payment Buttons */}
      {selectedPlan && (
        <div className="px-4 sm:px-6 lg:px-8 pb-16">
          <div className="max-w-6xl mx-auto">
            <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 px-8 py-8">
                ...
                <div className="flex flex-wrap gap-2 mt-6">
                  {selectedPlan.featuresAccess?.interviewScheduling && <span className="text-xs px-3 py-1 rounded-full font-medium bg-emerald-500/20 text-emerald-300">✓ Interview Scheduling</span>}
                  {selectedPlan.featuresAccess?.advancedAnalytics && <span className="text-xs px-3 py-1 rounded-full font-medium bg-emerald-500/20 text-emerald-300">✓ Advanced Analytics</span>}
                  {selectedPlan.featuresAccess?.prioritySupport && <span className="text-xs px-3 py-1 rounded-full font-medium bg-emerald-500/20 text-emerald-300">✓ Priority Support</span>}
                  {selectedPlan.razorpayPlanId && <span className="text-xs px-3 py-1 rounded-full font-medium bg-blue-500/20 text-blue-300">Razorpay Enabled</span>}
                </div>
              </div>

              {/* Feature Comparison Table - Unchanged */}
              <div className="p-6 md:p-8">
                {/* Your full feature comparison table code remains exactly as you wrote it */}
