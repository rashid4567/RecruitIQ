...
import { Briefcase, Code2, Star, TrendingUp } from "lucide-react";
...
const skillSuggestions = [
  ...
  "Java", "Go", "Redis", "Terraform", "CI/CD", "REST APIs",
  "System Design", "Microservices", "Vue.js", "Angular", "Swift",
  "Kotlin", "Flutter", "Django", "FastAPI", "Spring Boot",
];
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm ml-10">What should the ideal candidate bring to the table?</p>
      </div>

      <div className="space-y-7">
        {/* Experience Section */}
        <div className="p-5 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-100/80">
          ...
          <div className="grid grid-cols-4 gap-2 mb-4">
            {experienceLevels.map((level) => {
              ...
              return (
                ...
                >
                  <span className={`w-2 h-2 rounded-full ${level.color} mb-1.5`} />
                  <span className="font-bold text-xs">{level.label}</span>
                  <span className="text-[11px] text-gray-400">{level.range} yrs</span>
                </button>
              );
            })}
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  ...
                  value={expMin}
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, experienceMin: parseInt(e.target.value) || 0 }))
                  }
                  className={`h-11 rounded-xl border-2 bg-white text-sm font-medium pr-14 transition-all focus:ring-4 focus:ring-blue-100 ${
                    errors.experienceMin ? "border-red-400" : "border-blue-200 focus:border-blue-400"
                  }`}
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">years</span>
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold text-blue-700/80 uppercase tracking-wider block mb-1.5">
              ...
              <div className="relative">
                <Input
                  ...
                  value={expMax}
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, experienceMax: parseInt(e.target.value) || 0 }))
                  }
                  className={`h-11 rounded-xl border-2 bg-white text-sm font-medium pr-14 transition-all focus:ring-4 focus:ring-blue-100 ${
                    errors.experienceMax ? "border-red-400" : "border-blue-200 focus:border-blue-400"
                  }`}
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">years</span>
              </div>
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            {formData.requiredSkills.length > 0 && (
              <span className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full border border-indigo-100">
                {formData.requiredSkills.length} skill{formData.requiredSkills.length !== 1 ? "s" : ""} added
              </span>
            )}
          </div>
          <p className="text-xs text-gray-400 mb-2 ml-7">Skills candidates must have — these will be shown prominently on the listing</p>
          <TagInput
            tags={formData.requiredSkills}
            setTags={(tags) => setFormData((p) => ({ ...p, requiredSkills: tags }))}
            placeholder="Type a skill and press Enter..."
            suggestions={skillSuggestions.filter((s) => !formData.preferredSkills.includes(s))}
            color="indigo"
          />
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            {formData.preferredSkills.length > 0 && (
              <span className="text-xs font-semibold text-violet-600 bg-violet-50 px-2.5 py-1 rounded-full border border-violet-100">
                {formData.preferredSkills.length} skill{formData.preferredSkills.length !== 1 ? "s" : ""} added
              </span>
            )}
          </div>
          <p className="text-xs text-gray-400 mb-2 ml-7">Nice-to-have skills that would set candidates apart</p>
          <TagInput
            ...
            suggestions={skillSuggestions.filter((s) => !formData.requiredSkills.includes(s))}
            color="violet"
          />
        </div>
      </div>
